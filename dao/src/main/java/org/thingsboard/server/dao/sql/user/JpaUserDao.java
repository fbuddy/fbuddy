...
import org.thingsboard.server.common.data.UserFilterForm;
...
public class JpaUserDao extends JpaAbstractDao<UserEntity, User> implements UserDao {
    ...
    @Override
    public User findByUserName(TenantId tenantId, String email) {
        return DaoUtil.getData(userRepository.findByUserName(email));
    }
    ...
    @Override
    public PageData<User> findAllWithFilter(UserFilterForm userFilter, PageLink pageLink) {
        return DaoUtil.toPageData(userRepository.findAllWithFilter(pageLink.getTextSearch(),
                userFilter.getFirstName(),
                userFilter.getLastName(),
                userFilter.getEmail(),
                userFilter.getPhone(),
                userFilter.getUserType(),
                userFilter.getUserProfileType(),
                userFilter.getCompany(),
                userFilter.getDepartment(),
                userFilter.getDesignation(),
                userFilter.getUserName(),
                userFilter.getSerialNumber(),
                userFilter.getUserProfileStatus(),
                userFilter.getStartDate(),
                userFilter.getEndDate(),
                DaoUtil.toPageable(pageLink)));
    }
    @Override
    public PageData<User> findAllByRolesWithFilter(UserFilterForm userFilter, PageLink pageLink) {
        return DaoUtil.toPageData(userRepository.findAllByRolesWithFilter(pageLink.getTextSearch(),
                userFilter.getFirstName(),
                userFilter.getLastName(),
                userFilter.getEmail(),
                userFilter.getPhone(),
    ...
    public PageData<User> findAllByAuthority(Authority authority, PageLink pageLink) {
        return DaoUtil.toPageData(userRepository.findAllByAuthority(authority.name(), DaoUtil.toPageable(pageLink)));
    }
    ...
    @Override
    public User findBySerialNumber(TenantId tenantId, Integer serialNumber) {
        return DaoUtil.getData(userRepository.findBySerialNumber(serialNumber));
    }

    @Override
    public User findByEmailOrUserNameOrPhone(String userName) {
        return DaoUtil.getData(userRepository.findByEmailOrUserNameOrPhone(userName));
    }
    ...
    public List<User> findByIds(List<UUID> userIds) {
        ...
        List<UserEntity> userEntities = userRepository.findUsersByIds(userIds);
        for(UserEntity user : userEntities){
            users.add(DaoUtil.getData(user));
        }
        return users;
    }
