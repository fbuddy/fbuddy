...
public class JpaDeviceDao extends JpaAbstractDao<DeviceEntity, Device> implements DeviceDao {
    ...
    @Override
    public Optional<Device> findDeviceByMeterNo(Long meterNo) {
        Device device = DaoUtil.getData(deviceRepository.findByMeterNo(meterNo));
        return Optional.ofNullable(device);
    }
    ...
    @Override
    public Long findMaxMeterNo() {
        return deviceRepository.findMaxMeterNo();
    }

    @Override
    public Map<String, Map<String, Long>> findMeterSummary(MeterFilter meterFilter) {
        Map<String, Map<String, Long>> meterSummary = null;
        Map<String, Long> meterSummaryData = null;

        meterSummaryData = deviceRepository.findMeterSummary(meterFilter.getMeterType(), meterFilter.getHwVersion());
        if (meterSummaryData != null && !meterSummaryData.isEmpty()) {
            meterSummary = new HashMap<>();
            meterSummary.put(MeterInstallationStatus.STANDBY.getId(), CollectionsUtil.getCtPctData(meterSummaryData.get("standByCt"), meterSummaryData.get("standByPct")));
            meterSummary.put(MeterInstallationStatus.INSTALLED.getId(), CollectionsUtil.getCtPctData(meterSummaryData.get("installedCt"), meterSummaryData.get("installedPct")));
            meterSummary.put(MeterInstallationStatus.UNINSTALL_IN_PROGRESS.getId(), CollectionsUtil.getCtPctData(meterSummaryData.get("uninstallInProgressCt"), meterSummaryData.get("uninstallInProgressPct")));
            meterSummary.put(MeterInstallationStatus.UNACCOUNTED.getId(), CollectionsUtil.getCtPctData(meterSummaryData.get("unaccountedCt"), meterSummaryData.get("unaccountedPct")));
            meterSummary.put(MeterInstallationStatus.IN_SERVICING.getId(), CollectionsUtil.getCtPctData(meterSummaryData.get("inServicingCt"), meterSummaryData.get("inServicingPct")));
            meterSummary.put(MeterInstallationStatus.RETIRED.getId(), CollectionsUtil.getCtPctData(meterSummaryData.get("retiredCt"), meterSummaryData.get("retiredPct")));
        }
        return meterSummary;
    }

    @Override
    public PageData<Device> findMeter(MeterFilter meterFilter, PageLink pageLink) {
        List<Long> meterNos = null;

        if (meterFilter.getMeterNo() != null)
            meterNos = TypeCastUtil.getSerialNumberData(meterFilter.getMeterNo());

        return DaoUtil.toPageData(deviceRepository.findMeter(
                ...
                meterFilter.getProductionDateFrom(), meterFilter.getProductionDateTo(),
                DaoUtil.toPageable(pageLink)));
