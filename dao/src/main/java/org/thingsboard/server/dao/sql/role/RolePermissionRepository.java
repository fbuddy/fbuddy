...
public interface RolePermissionRepository extends JpaRepository<RolePermissionEntity, UUID> {
    ...
    @Query("select rpe from RolePermissionEntity rpe where rpe.roleId = :roleId ")
    List<RolePermissionEntity> findAllPermissionInfoByRoleId(UUID roleId);

    List<RolePermissionEntity> findByModuleId(UUID id);
}
