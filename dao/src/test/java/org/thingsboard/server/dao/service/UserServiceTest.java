...
public class UserServiceTest extends AbstractServiceTest {
    ...
    public void before() {
        ...
        List<Role> roleList = roleService.findAllRoleByFilter(new PageLink(10), new RoleFilter()).getData();
        tenantAdmin.getRoleIds().add(roleList.get(0).getId());
    ...
    public void testFindUserByEmail() {
        ...
        Assert.assertTrue(user.getNewAuthorities().contains(Authority.SYS_ADMIN));
        user = userService.findUserByEmail(tenantId, "tenant@thingsboard.org");
        Assert.assertNotNull(user);
        user = userService.setUserRolesAndAuthorities(user);
        Assert.assertTrue(user.getNewAuthorities().contains(Authority.TENANT_ADMIN));
        user = userService.findUserByEmail(tenantId, "customer@thingsboard.org");
        Assert.assertNotNull(user);
        user = userService.setUserRolesAndAuthorities(user);
        Assert.assertTrue(user.getNewAuthorities().isEmpty());
