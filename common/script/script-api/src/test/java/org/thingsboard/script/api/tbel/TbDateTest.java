...
class TbDateTest {
    ...
    void testToISOStringThreadLocalStaticFormatter() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        int date = 29;
        int month = 2;
        String pattern = "2024-%s-%sT%s:%s:42.987Z";
        String tsStr = String.format(pattern, String.format("%02d", month), date, hrs, minutes);  //Thu Feb 29 2024 14:35:42.987 GMT+0000
        ...
        assertThat(executor.submit(tbDate::toISOString).get(30, TimeUnit.SECONDS))
                ...
                .startsWith(datePrefix);
        TbDateTestEntity tbDateTest = new TbDateTestEntity(tbDate.getFullYear(), month, date,hrs + offsetLocalSecond/60/60, minutes + (offsetLocalSecond/60)%60);
        String datePrefixLocal = String.format(pattern, tbDateTest.geMonthStr(), tbDateTest.geDateStr(), tbDateTest.geHoursStr(), tbDateTest.geMinutesStr());
    ...
    void testToLocaleDateString() {
        ...
        int localOffsetMins = (ZoneId.systemDefault().getRules().getOffset(d.getInstant()).getTotalSeconds()/60)%60;
        //if(localOffsetmins>)
        int hrs = 14 - localOffsetHrs;
        int mins = 15 - localOffsetMins;
        String expected = "";
        if(mins>0) {
            expected = "2022-10-09T" + String.format("%02d", hrs) + ":" + mins + ":30Z";
            Assert.assertEquals(expected, d.toISOString());
        }
        else if(mins<0){
            expected = "2022-10-09T" + String.format("%02d",(hrs-1)) + ":" + (mins+60) + ":30Z";
            Assert.assertEquals(expected, d.toISOString());
        }
        ...
        localOffsetMins = (ZoneId.systemDefault().getRules().getOffset(d.getInstant()).getTotalSeconds()/60)%60;
        hrs = 14 - localOffsetHrs;
        mins = 15 - localOffsetMins;
        //expected = "2023-10-04T" + hrs + ":15:30Z";
        //Assert.assertEquals(expected, d.toISOString());

        if(mins>0) {
            expected = "2023-10-04T" + String.format("%02d", hrs) + ":" + mins + ":30Z";

            Assert.assertEquals(expected, d.toISOString());
        }
        else if(mins<0){
            expected = "2023-10-04T" + String.format("%02d",(hrs-1)) + ":" + (mins+60) + ":30Z";
            Assert.assertEquals(expected, d.toISOString());
        }
    ...
    void Test_DateString_With_TZ() {
        ...
        int hrs = 8;
        int mins = 4;
        String pattern = "2023-09-%s %s:%s:05";
        String tzStr = "-04:00:00";
        String dateStr = "2023-09-0" + date + "T0" + hrs + ":" + String.format("%02d",mins) + ":05.123" + tzStr;
        TbDate d = new TbDate(dateStr);
        int localOffsetHrs = ZoneId.systemDefault().getRules().getOffset(d.getInstant()).getTotalSeconds()/60/60;
        int localOffsetMins = (ZoneId.systemDefault().getRules().getOffset(d.getInstant()).getTotalSeconds()/60)%60;
        TbDateTestEntity tbDateTest = new TbDateTestEntity(23, 9, date, hrs + localOffsetHrs - tz, mins + localOffsetMins);
        String expected = String.format(pattern, tbDateTest.geDateStr(), tbDateTest.geHoursStr(), tbDateTest.geMinutesStr());
        Assert.assertEquals(expected, d.toLocaleString());

        tz = +5;
        tzStr = "+05:00";
        dateStr = "2023-09-0" + date + "T0" + hrs + ":" + String.format("%02d",mins) + ":05.123" + tzStr;
        d = new TbDate(dateStr);
        localOffsetHrs = ZoneId.systemDefault().getRules().getOffset(d.getInstant()).getTotalSeconds()/60/60;
        localOffsetMins = (ZoneId.systemDefault().getRules().getOffset(d.getInstant()).getTotalSeconds()/60)%60;
        tbDateTest = new TbDateTestEntity(23, 9, date, hrs + localOffsetHrs - tz, mins + localOffsetMins);
        expected = String.format(pattern, tbDateTest.geDateStr(), tbDateTest.geHoursStr(), tbDateTest.geMinutesStr());
        Assert.assertEquals(expected, d.toLocaleString());
    ...
    void Test_Get_LocalDateTime_With_TZ() {
        ...
        String pattern = "2023-09-%s %s:%s:05";
        String tzStr = "UTC";
        TbDate d = new TbDate(23, 9, date, hrs, mins, 5);
        int localOffsetHrs = 0;
        int localOffsetMins = 0;
        TbDateTestEntity tbDateTest = new TbDateTestEntity(23, 9, date, hrs + localOffsetHrs - tz, mins + localOffsetMins);
        String expected = String.format(pattern, tbDateTest.geDateStr(), tbDateTest.geHoursStr(), tbDateTest.geMinutesStr());
        Assert.assertEquals(expected, d.toLocaleString());

        d = new TbDate(23, 9, date, hrs, mins, 5, tzStr);
        localOffsetHrs = ZoneId.systemDefault().getRules().getOffset(d.getInstant()).getTotalSeconds()/60/60;
        localOffsetMins = (ZoneId.systemDefault().getRules().getOffset(d.getInstant()).getTotalSeconds()/60)%60;
        tbDateTest = new TbDateTestEntity(23, 9, date, hrs + localOffsetHrs - tz, mins + localOffsetMins);
        expected = String.format(pattern, tbDateTest.geDateStr(), tbDateTest.geHoursStr(), tbDateTest.geMinutesStr());
    ...
    private void testResultChangeDateTime(TbDate d) {
        ...
        TbDateTestEntity tbDateTestEntity = new TbDateTestEntity(d.getFullYear(), d.getMonth(), d.getDate(), (d.getHours() - (localOffset/60/60)), (d.getMinutes() - (localOffset/60)%60));
        ...
        Assert.assertEquals(tbDateTestEntity.getMinutes(), d.getUTCMinutes());
