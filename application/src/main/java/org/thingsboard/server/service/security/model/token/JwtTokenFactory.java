...
public class JwtTokenFactory {
    ...
    public AccessJwtToken createAccessJwtToken(SecurityUser securityUser) {
        if (securityUser.getAuthorities().isEmpty()) {
        ...
        jwtBuilder.claim(FIRST_NAME, securityUser.getFirstName())
                ...
                .claim(USER_NAME, securityUser.getUserName())
    ...
    public SecurityUser parseAccessJwtToken(String token) {
        ...
        securityUser.setNewAuthorities(scopes.stream().collect(Collectors.toSet()));
        String tenantId = claims.get(TENANT_ID, String.class);
        if (tenantId != null) {
            securityUser.setTenantId(TenantId.fromUUID(UUID.fromString(tenantId)));
        } else if (securityUser.getAuthorities().contains(new SimpleGrantedAuthority(Authority.SYS_ADMIN.name()))) {
        ...
        if (securityUser.getNewAuthorities().size()==1 && !securityUser.getAuthorities().contains(new SimpleGrantedAuthority(Authority.PRE_VERIFICATION_TOKEN.name()))) {
