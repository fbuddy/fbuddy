...
public class RestAuthenticationProvider implements AuthenticationProvider {
    ...
    private SecurityUser authenticateByUsernameAndPassword(Authentication authentication, UserPrincipal userPrincipal, String username, String password) {
        User user = userService.findByEmailOrUserNameOrPhone(TenantId.SYS_TENANT_ID, username);
        if (user == null) {
            throw new UsernameNotFoundException("User not found: " + username);
        }
        user = userService.setUserRolesAndAuthorities(user);
        try {
            if (!UserProfileStatus.ACTIVE.equals(user.getUserProfileStatus())) {
                throw new UsernameNotFoundException("User is in-active: " + username);
            }
            if (UserProfileType.COMPUTER.equals(user.getUserProfileType())) {
            ...
            if (user.getNewAuthorities().isEmpty())
