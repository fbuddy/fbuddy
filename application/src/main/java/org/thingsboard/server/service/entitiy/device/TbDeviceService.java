...
public interface TbDeviceService {
    ...
    Device saveMeter(Meter meter, String accessToken, User user) throws ThingsboardException;

    void updateMeterData(Meter oldMeter, Meter newMeter, User user);

    Map<String, Map<String, Long>> getMeterSummary(MeterFilter meterFilter);

    PageData<Device> getMeter(MeterFilter meterFilter, PageLink pageLink);
}
