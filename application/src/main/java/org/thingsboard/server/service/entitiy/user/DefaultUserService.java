    ...
    public User save(TenantId tenantId, CustomerId customerId, User tbUser, boolean sendActivationMail,
        ...
        try {
            ...
            /*if(ActionType.ADDED.equals(actionType)) {
                ...
                int rand_int1 = 0;
                while(rand_int1 == 0) {
                    rand_int1 = rand.nextInt(MAX_ID - MIN_ID + 1) + MIN_ID;
                    User existingUserBySerialNumber = userService.findUserBySerialNumber(TenantId.SYS_TENANT_ID, rand_int1);
                    if(existingUserBySerialNumber != null){
                        rand_int1 = 0;
                    }
                }
                tbUser.setSerialNumber(rand_int1);
            }*/
            User savedUser = checkNotNull(userService.saveUser(tenantId, tbUser));
            if(UserProfileType.HUMAN.equals(savedUser.getUserProfileType()) ) {
                if (sendEmail) {
