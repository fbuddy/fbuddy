    ...
    protected SecurityUser getOrCreateSecurityUserFromOAuth2User(OAuth2User oauth2User, OAuth2Registration registration) {
        ...
        if (user == null) {
            ...
            try {
                ...
                if (user == null) {
                    user = new User();
                    user.setUserProfileStatus(UserProfileStatus.IN_ACTIVE);
                    ...
                    user.setPhone(oauth2User.getEmail());
                ...
                log.error("Can't get or create new security user from oauth2 user", e);
                throw new RuntimeException(e.getMessage()+" Can't get or create new security user from oauth2 user", e);
            } finally {
        ...
        try {
            user = userService.setUserRolesAndAuthorities(user);
            ...
            log.error("Can't get security user from oauth2 user", e);
            throw new RuntimeException(e.getMessage()+" Can't get security user from oauth2 user", e);
        }
    }
