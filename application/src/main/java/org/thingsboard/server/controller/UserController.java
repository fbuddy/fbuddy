...
import org.thingsboard.server.dao.user.UserProfileImageService;
...
public class UserController extends BaseController {
    ...
    //public static final String ACTIVATE_URL_PATTERN = "%s/login/createPassword?activateToken=%s";
    ...
    public User getUserById(
        ...
        if(authUser.getId().equals(userId)) {
            user = userService.findUserById(authUser.getTenantId(),userId);
        }
        else
        ...
        if (user.getAdditionalInfo().isObject()) {
            ...
            if (userCredentials != null && userCredentials.isEnabled() && !additionalInfo.has("userCredentialsEnabled")) {
    ...
    public void sendActivationEmail(
        ...
        if(UserProfileType.HUMAN.equals(user.getUserProfileType()) && UserProfileStatus.ACTIVE.equals(user.getUserProfileStatus())) {
            ...
            throw new ThingsboardException("User is not allowed to login!", ThingsboardErrorCode.BAD_REQUEST_PARAMS);
        }
    }
    ...
    public PageData<User> getUsers(
        ...
        if (currentUser.getNewAuthorities().contains(Authority.TENANT_ADMIN.name())) {
    ...
    @PreAuthorize("hasAnyAuthority('SYS_ADMIN','USER_MANAGEMENT_SEARCH_EDIT','USER_MANAGEMENT_EDIT','USER_MANAGEMENT_SEARCH_VIEW')")
    @RequestMapping(value = "/sys-admin/users", params = {"pageSize", "page"}, method = RequestMethod.GET)
    @ResponseBody
    public PageData<User> getSysAdmins(
            ...
            @ApiParam(value = USER_TEXT_SEARCH_DESCRIPTION)
            @RequestParam(required = false) String textSearch,
            @Valid UserFilterForm userFilter,
            @ApiParam(value = SORT_PROPERTY_DESCRIPTION, allowableValues = USER_SORT_PROPERTY_ALLOWABLE_VALUES)
            @RequestParam(required = false) String sortProperty,
            @ApiParam(value = SORT_ORDER_DESCRIPTION, allowableValues = SORT_ORDER_ALLOWABLE_VALUES)
            @RequestParam(required = false) String sortOrder) throws ThingsboardException {
        PageLink pageLink = createPageLink(pageSize, page, textSearch, sortProperty, sortOrder);
        return checkNotNull(userService.findAllUsersWithFilters(userFilter, pageLink));
    }

    @PreAuthorize("hasAnyAuthority('SYS_ADMIN','USER_MANAGEMENT_SEARCH_EXPORT')")
    @GetMapping("/sys-admin/users/export-to-csv")
    public void exportIntoCSV(
        ...
        PageLink pageLink = createPageLink(9999999, 0, textSearch, sortProperty, sortOrder);
        response.setContentType("text/csv");
        response.addHeader("Content-Disposition", "attachment; filename=\"user.csv\"");
        csvGenerator.writeUsersToCsv(userService.findAllUsersWithFilters(userFilter, pageLink).getData(), response.getWriter());
    }
    ...
    @ApiOperation(value = "Get Users with SYS_ADMIN Authority",
            notes = "Returns a list of users with SYS_ADMIN authority filtered by the specified role."
    )
    @GetMapping(value = "/sys-admin/authority")
    @PreAuthorize("isAuthenticated()")
    public List<User> getSysAdminUsers() throws ThingsboardException {
        PageLink pageLink = new PageLink(9999999, 0, "");
        return userService.findSysAdmins(pageLink).getData();
        //return userService.getUsersByRoleAndAuthority(currentUser.getTenantId());
    }

    @Autowired
    private UserProfileImageService userProfileImageService;

    @ApiOperation(value = "User Profile Image Upload for user management(UserProfileImage)",
            notes = "Returns a User Profile ImageUpload object.")
    @PreAuthorize("hasAnyAuthority('SYS_ADMIN')")
    @RequestMapping(value = "/user/profile/{userId}", method = RequestMethod.POST)
    @ResponseBody
    public UserProfileImage getUserProfileImage(@RequestPart MultipartFile file, @PathVariable("userId") String userId) throws ThingsboardException, Exception {
        // SecurityUser currentUser = getCurrentUser();
        String originalFile = Base64.getEncoder().encodeToString(file.getBytes());
        UserId userIdObject = new UserId(UUID.fromString(String.valueOf(userId)));
        long currentTime = System.currentTimeMillis();
        UserProfileImage userProfileImage = new UserProfileImage();
        userProfileImage.setImage(originalFile);
        userProfileImage.setUserId(userIdObject);
        userProfileImage.setCreated_time(currentTime);
        return checkNotNull(userProfileImageService.userImage(userProfileImage));
    }

    @ApiOperation(value = "Get Image for user management(uploadImage)",
            notes = "Returns a ImageUpload object.")
    @RequestMapping(value = "/user/profile/{userId}", method = RequestMethod.GET)
    @ResponseBody
    @PreAuthorize("isAuthenticated()")
    public UserProfileImage getUserProfileImage(@PathVariable("userId") String userId) throws ThingsboardException, Exception {
        SecurityUser currentUser = getCurrentUser();
        return userProfileImageService.getImage(userId);
    }
